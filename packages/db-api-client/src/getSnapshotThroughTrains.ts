...
function matchesRoute(trainRoute: string[], observationStation: string, requiredRouteStops: string[]) {
  ...
  if (observationIndex >= 0 && (observationIndex < ordered[0].index || observationIndex > ordered[ordered.length - 1].index)) return false;
  return true;
}
...
export async function getSnapshotThroughTrains(db: Client, crossing: Crossing): Promise<SnapshotThroughTrain[] | null> {
  try {
    ...
    for (const row of result.rows as any[]) {
      ...
      const train = { line: String(row.line || ""), category: String(row.category || ""), route, origin: row.origin ? String(row.origin) : undefined, destination: row.destination ? String(row.destination) : undefined };
