...
function matchesOsmCorridor(
  ...
  const infrastructureRefs = requiredRouteStops.filter((stop) =>
    /^\d{2,6}$/.test(String(stop).trim()),
  );

  // Numeric route references describe infrastructure, not a station name.
  // They are therefore deliberately not treated as a route match by themselves.
  // The crossing-specific OSM topology check later decides whether the train
  // really traverses the crossing corridor.
...
function ruleAllowsTrain(rule: any, train: MobilithekTrainEvent) {
  ...
  const legacyLongDistance =
    categories.length === 3 &&
    categories.includes("ICE") &&
    categories.includes("IC") &&
    categories.includes("EC");
  if (legacyLongDistance) return true;
  if (!categories.length) return true;
  return categories.includes(train.category) ||
    categories.some((category: string) => train.line.toUpperCase().includes(String(category).toUpperCase()));
}
    ...
    try {
      ...
      const names = Array.from(
        new Set(
          (data?.elements || [])
            .map((element: any) =>
              String(element?.tags?.name || element?.tags?.["name:de"] || "").trim(),
            )
            .filter(Boolean),
        ),
      ) as string[];
  ...
  return Array.from(
    new Set(
      candidates
        ...
        .filter((value) => normalizeStationName(value).length >= 4),
    ),
  );
}

function buildMobilithekCandidates(
  ...
  const rules = baseRules.length
    ? baseRules
    : dynamicStops.map((station) => ({
        observationEva: `transit:${normalizeStationName(station)}`,
        observationStation: station,
        categories: [],
        trackDistanceMeters: 0,
        fallbackOffsetSeconds: 180,
        direction: "unknown",
      }));
  ...
  for (const rule of rules) {
    for (const train of events) {
      ...
      if (
        crossingTime.getTime() < Date.now() - 60000 ||
        crossingTime.getTime() > Date.now() + 3 * 60 * 60_000
      ) continue;
  ...
  for (const list of byTrain.values()) {
    ...
    const sorted = [...list].sort(
      (a, b) => Date.parse(a.observationActualTime) - Date.parse(b.observationActualTime),
    );
...
export async function getThroughTrains(crossing: Crossing): Promise<ThroughTrain[]> {
  ...
  try {
    ...
    const dynamicStops = rules.length
      ? []
      : Array.from(new Set([
          ...(await discoverTransitStops(crossing)),
          ...crossingNameCandidates(crossing),
        ]));
    const candidates = buildMobilithekCandidates(registry, crossing, dynamicStops);
    if (candidates.length) return candidates;
  } catch (error) {
    console.warn(
      "Mobilithek through-train registry unavailable; falling back to DB Timetables API",
      error,
    );
  }
  ...
  const uniqueEvas = Array.from(
    new Set(rules.map((rule) => String(rule.observationEva).trim()).filter(Boolean)),
  );
  ...
  for (const rule of rules) {
    ...
    for (const train of events) {
      ...
      if (!matchesOsmCorridor(route, rule.observationStation, crossing.requiredRouteStops || [], isLocalTransitTrain(train as any))) continue;
