...
function matchesOsmCorridor(trainRoute: string[], observationStation: string, requiredRouteStops: string[], transit = false) {
  ...
  // Stadtbahn/Tram journeys often arrive from a Mobilithek route without the
  // two long-distance rail anchors used by the original BÜ rules. If the
  // observation station itself is on the journey, that is sufficient here;
  // the authoritative OSM topology check runs afterwards in the status API.
