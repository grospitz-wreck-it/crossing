...
function matchesOsmCorridor(trainRoute: string[], observationStation: string, requiredRouteStops: string[], transit = false) { if (!trainRoute.length) return false; const anchors = requiredRouteStops.map((stop, order) => ({ stop, order, index: routeIndex(trainRoute, stop) })).filter((entry) => entry.index >= 0); if (anchors.length < 2) return transit && routeIndex(trainRoute, observationStation) >= 0; const ordered = [...anchors].sort((a, b) => a.order - b.order); for (let i = 1; i < ordered.length; i += 1) if (ordered[i - 1].index >= ordered[i].index) return false; const observationIndex = routeIndex(trainRoute, observationStation); if (observationIndex >= 0) { const first = ordered[0].index; const last = ordered[ordered.length - 1].index; if (observationIndex < first || observationIndex > last) return false; } return true; }
...
async function discoverTransitStops(crossing: Crossing): Promise<string[]> {
  const lat = Number((crossing as any).lat);
  const lon = Number((crossing as any).lon);
  if (!Number.isFinite(lat) || !Number.isFinite(lon)) return [];
  for (const endpoint of ["https://overpass-api.de/api/interpreter", "https://overpass.kumi.systems/api/interpreter"]) {
    try {
      const query = `[out:json][timeout:15];(node(around:2500,${lat},${lon})[public_transport=platform];node(around:2500,${lat},${lon})[railway=tram_stop];node(around:2500,${lat},${lon})[railway=halt];);out tags;`;
      ...
      const names = Array.from(new Set((data?.elements || []).map((element: any) => String(element?.tags?.name || element?.tags?.["name:de"] || "").trim()).filter(Boolean)));
...
function crossingNameCandidates(crossing: Crossing): string[] {
  const raw = String(crossing.name || "").replace(/bahnübergang|bahnuebergang|bue|bü/gi, " ");
  const parts = raw.split(/\s{2,}|[|,;]+/).map((part) => part.trim()).filter(Boolean);
  const candidates = parts.length > 1 ? parts : raw.split(/\s+/).filter((part) => part.length >= 4);
  return Array.from(new Set(candidates.map((value) => value.trim()).filter((value) => normalizeStationName(value).length >= 4)));
}

function buildMobilithekCandidates(events:MobilithekTrainEvent[],crossing:Crossing, dynamicStops:string[]=[]):ThroughTrain[]{
  ...
  const rules = baseRules.length ? baseRules : dynamicStops.map((station) => ({ observationEva: `transit:${normalizeStationName(station)}`, observationStation: station, categories: [], trackDistanceMeters: 0, fallbackOffsetSeconds: 180, direction: "unknown" }));
...
export async function getThroughTrains(crossing:Crossing):Promise<ThroughTrain[]>{
  ...
  try{const registry=await getMobilithekTrainRegistry();const dynamicStops=rules.length?[]:await discoverTransitStops(crossing);const candidates=buildMobilithekCandidates(registry,crossing,dynamicStops);if(candidates.length)return candidates;}
