...
import EditCrossingDrawer from "./EditCrossingDrawer";
...
function fmtCountdown(seconds: number) { if (seconds < 60) return `in ${Math.max(1, seconds)} Sek.`; return `in ${Math.floor(seconds / 60)} Min.`; }
...
export default function ForecastOverlay() {
  ...
  return <>
    <div onMouseDown={(e) => { if (e.target === e.currentTarget) setOpen(false); }} style={{ position: "fixed", inset: 0, zIndex: 300, background: "rgba(15,20,28,.42)", display: "flex", justifyContent: "flex-end" }}>
      <aside style={{ width: "min(620px,100vw)", height: "100%", background: "#fff", boxShadow: "-25px 0 70px rgba(0,0,0,.2)", display: "flex", flexDirection: "column" }}>
        <header style={{ padding: "26px 28px 20px", borderBottom: "1px solid #eceef1", display: "flex", justifyContent: "space-between", gap: 20 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}><button onClick={() => data?.crossing?.id && setEditId(data.crossing.id)} disabled={!data?.crossing?.id} style={{ border: 0, background: "#e10600", color: "#fff", borderRadius: 10, padding: "0 13px", fontWeight: 800, cursor: "pointer" }}>✎ Bearbeiten</button><button onClick={() => setOpen(false)} style={{ border: 0, background: "#f1f2f4", borderRadius: 10, width: 38, height: 38, fontSize: 24, cursor: "pointer" }}>×</button></div>
        </header>
        <div style={{ overflow: "auto", padding: 28, display: "grid", gap: 18 }}>
          ...
          {!loading && !error && data && <>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}><div style={{ padding: 16, borderRadius: 12, background: data.state === "CLOSED" ? "#fff4f4" : "#f4fbf6" }}><small style={{ color: "#68717b" }}>Status</small><strong style={{ display: "block", marginTop: 5, fontSize: 20 }}>{data.state === "CLOSED" ? "GESCHLOSSEN" : data.state || "UNBEKANNT"}</strong></div><div style={{ padding: 16, borderRadius: 12, background: "#f7f8fa" }}><small style={{ color: "#68717b" }}>Nächste Schließung</small><strong style={{ display: "block", marginTop: 5, fontSize: 20 }}>{next ? fmtTime(next.start) : "–"}</strong>{next && <span style={{ color: "#68717b", fontSize: 12 }}>{fmtCountdown(next.closeInSeconds)}</span>}</div></div>
    ...
    {editId && <EditCrossingDrawer crossingId={editId} onClose={() => setEditId("")} onSaved={() => { setEditId(""); setOpen(false); }} />}
  </>;
}
