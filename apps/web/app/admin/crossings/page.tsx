...
export default function CrossingsAdmin() {
  ...
  async function save() {
    ...
    try {
      ...
      const referenceRes = await fetch(`/api/admin/crossings/${encodeURIComponent(savedId)}/reference-lines`, { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ referenceLines }) });
      const referenceData = await referenceRes.json().catch(() => ({}));
      if (!referenceRes.ok) throw new Error(referenceData.error || `Referenzlinien konnten nicht gespeichert werden (${referenceRes.status}).`);
  ...
  async function openForecast(crossing: Crossing) {
    ...
    try {
      const res = await fetch(`/api/crossings/${encodeURIComponent(crossing.id)}/status`, { cache: "no-store" });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error || `Prognose konnte nicht geladen werden (${res.status}).`);
      const stations = Array.isArray(crossing.stations) ? crossing.stations.map((station) => ({ eva: station.eva, stationName: station.stationName, ok: true, count: 0 })) : [];
      setForecast({ ...data, nextClosure: data.phase ? { ...data.phase, closeInSeconds: Number(data.nextCloseIn || 0), openInSeconds: Number(data.nextOpenIn || 0) } : null, stations });
    } catch (e) {
      setForecastError(e instanceof Error ? e.message : "Prognose konnte nicht geladen werden.");
    } finally {
