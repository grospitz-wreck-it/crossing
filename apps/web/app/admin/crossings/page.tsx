...
export default function CrossingsAdmin() {
  ...
  function resetWizard() {
    setCoords(""); setLocation(null); setLookupError(""); setNearbyStations([]); setRailwayInfrastructure({ status: "NOT_RUN", candidates: [] }); setSelectedRouteKey(""); setSelectedRoute(null);
    setForm({ id: "", name: "", eva: "", lat: "", lon: "", closeOffsetSeconds: "80", openOffsetSeconds: "20", confidence: "0.5", requiredRouteStops: "", stations: [] });
  }

  async function resolveLocation(value = coords) {
    const input = value.trim(); if (!looksLikeLocation(input)) return;
    const requestId = ++lookupRequest.current;
    setLookupError(""); setStationLoading(true); setNearbyStations([]); setRailwayInfrastructure({ status: "LOADING", candidates: [] }); setSelectedRouteKey(""); setSelectedRoute(null);
    try {
      ...
      setLocation(resolvedLocation); setNearbyStations(data.stations || []);
      if (!resolvedLocation) return;
      setForm((f) => ({ ...f, lat: String(resolvedLocation.lat), lon: String(resolvedLocation.lon) }));
      try {
        const infraRes = await fetch(`/api/admin/crossings/infrastructure?lat=${encodeURIComponent(resolvedLocation.lat)}&lon=${encodeURIComponent(resolvedLocation.lon)}`, { cache: "no-store" });
        const infraData = await infraRes.json().catch(() => ({}));
        if (requestId !== lookupRequest.current) return;
        const candidates = Array.isArray(infraData?.candidates) ? infraData.candidates : [];
        setRailwayInfrastructure({ ...(infraData || {}), candidates });
        const first = candidates[0] || null;
        if (first) { setSelectedRouteKey(routeKey(first)); setSelectedRoute(first); }
      } catch {
        if (requestId === lookupRequest.current) setRailwayInfrastructure({ status: "ERROR", candidates: [], error: "OSM-Bahnstrecken konnten nicht geladen werden." });
      }
    } catch {
      if (requestId === lookupRequest.current) { setRailwayInfrastructure({ status: "ERROR", candidates: [] }); setLookupError("Standort konnte nicht geladen werden. Bitte erneut versuchen."); }
  ...
  function handleLocationChange(value: string) {
    ...
    lookupTimer.current = setTimeout(() => { void resolveLocation(value); }, 650);
  }

  function selectRoute(candidate: RailwayCandidate) {
  ...
  async function save() {
    ...
    try {
      ...
      const payload = {
        ...
        closeOffsetSeconds: Number(form.closeOffsetSeconds), openOffsetSeconds: Number(form.openOffsetSeconds), confidence: Number(form.confidence),
        routeRef: route?.ref || "", routeName: route?.name || "", selectedRouteRef: route?.ref || "", selectedRouteName: route?.name || "",
        selectedRoute: route ? { ...route, segments: route.segments } : null,
        requiredRouteStops: route?.ref ? [route.ref] : [],
        stations: form.stations.filter((s) => s.eva.trim()),
      };
      ...
      const savedId = String(data?.crossing?.id || data?.id || "").trim();
      if (!savedId) throw new Error("Der Server hat keinen Datensatz bestätigt.");
      const verify = await fetch(`/api/admin/crossings/${encodeURIComponent(savedId)}/forecast`, { cache: "no-store" });
      if (!verify.ok) throw new Error("Der Übergang wurde nicht sauber bestätigt. Der Wizard bleibt geöffnet.");
      await load();
      setOpen(false); resetWizard();
    } catch (e) {
      setLookupError(e instanceof Error ? e.message : "Speichern fehlgeschlagen");
    } finally { setSaving(false); }
  }

  async function openForecast(crossing: Crossing) {
    setForecast(null); setForecastError(""); setForecastLoading(true);
    try {
      const res = await fetch(`/api/admin/crossings/${encodeURIComponent(crossing.id)}/forecast`, { cache: "no-store" });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.error || "Prognose konnte nicht geladen werden.");
      setForecast(data);
    } catch (e) { setForecastError(e instanceof Error ? e.message : "Prognose konnte nicht geladen werden."); }
    finally { setForecastLoading(false); }
  }

  return <main className={styles.page}>
    <header className={styles.header}><div><div className={styles.eyebrow}>CROSSINGS · ADMIN</div><h1>Bahnübergänge</h1><p>Standorte, EVAs und zugehörige DB-Stationen verwalten.</p></div><button className={styles.primary} onClick={() => { resetWizard(); setOpen(true); }}>+ Übergang anlegen</button></header>
    <section className={styles.tableCard}><div className={styles.tableHead}><span>Übergang</span><span>EVA</span><span>Position</span><span>Stationen</span><span>Status</span></div>{crossings.map((c) => <button type="button" className={styles.row} key={c.id} onClick={() => void openForecast(c)} style={{ width: "100%", textAlign: "left", border: 0, cursor: "pointer" }}><div><strong>{c.name}</strong><small>{c.id}</small></div><code>{c.eva || "—"}</code><span>{Number(c.lat).toFixed(5)}, {Number(c.lon).toFixed(5)}</span><span>{c.stations?.length ?? 0} verknüpft</span><span className={styles.badge}>{c.status}</span></button>)}{!crossings.length && <div className={styles.empty}>Noch keine Übergänge in der Datenbank.</div>}</section>

    {forecast && <div className={styles.backdrop} onMouseDown={() => setForecast(null)}><aside className={styles.drawer} onMouseDown={(e) => e.stopPropagation()}><div className={styles.drawerHead}><div><div className={styles.eyebrow}>PROGNOSE</div><h2>{forecast.crossing?.name || "Bahnübergang"}</h2></div><button className={styles.close} onClick={() => setForecast(null)}>×</button></div><div className={styles.content}><div className={styles.location}><strong>{forecast.state === "CLOSED" ? "● GESCHLOSSEN" : "● OFFEN"}</strong><span>{forecast.crossing?.lat?.toFixed?.(6)}, {forecast.crossing?.lon?.toFixed?.(6)}</span></div>{forecast.message && <div className={styles.emptySmall}>{forecast.message}</div>}{forecast.nextClosure && <section><label>Nächste Schließzeit</label><div className={styles.nearbyStation}><div><strong>{formatTime(forecast.nextClosure.start)} – {formatTime(forecast.nextClosure.end)}</strong><span>{formatRelative(forecast.nextClosure.start)} · {forecast.nextClosure.trains?.length || 0} Zug/Züge</span></div></div></section>}<section><label>Weitere Prognosen</label>{forecast.closures?.length ? <div className={styles.nearbyStations}>{forecast.closures.map((closure: any, index: number) => <div className={styles.nearbyStation} key={`${closure.start}-${index}`}><div><strong>{formatTime(closure.start)} – {formatTime(closure.end)}</strong><span>{closure.trainCount} Zug/Züge · ca. {closure.durationMinutes} Min.</span></div></div>)}</div> : <div className={styles.emptySmall}>Keine weiteren Schließzeiten im Prognosefenster.</div>}</section><section><label>Datenbasis</label>{forecast.stations?.length ? <div className={styles.nearbyStations}>{forecast.stations.map((station: any) => <div className={styles.nearbyStation} key={station.eva}><div><strong>{station.stationName}</strong><span>EVA {station.eva} · {station.ok ? `${station.count} Fahrplanereignisse` : `Fehler: ${station.error || "unbekannt"}`}</span></div></div>)}</div> : <div className={styles.emptySmall}>Keine DB-Beobachtungsstation verknüpft.</div>}</section></div></aside></div>}
    ...
    {open && <div className={styles.backdrop} onMouseDown={() => !saving && setOpen(false)}><aside className={styles.drawer} onMouseDown={(e) => e.stopPropagation()}>
      <div className={styles.drawerHead}><div><div className={styles.eyebrow}>NEUER DATENSATZ</div><h2>Übergang einrichten</h2></div><button className={styles.close} disabled={saving} onClick={() => setOpen(false)}>×</button></div>
      ...
      <div className={styles.content}>
        ...
        {location && <section><label>DB-Stationen in der Umgebung</label><small>Vorläufige Vorschau. Nach Auswahl der Strecke werden diese Stationen auf den Streckenverlauf eingeschränkt.</small>{stationLoading ? <div className={styles.stationLoading}>DB-Stationen werden gesucht…</div> : nearbyStations.length ? <div className={styles.nearbyStations}>{nearbyStations.map((s) => { const selected = form.stations.some((x) => x.eva === s.eva); return <div className={`${styles.nearbyStation} ${selected ? styles.selected : ""}`} key={s.eva}><div><strong>{s.stationName}</strong><span>EVA {s.eva}{s.ril100 ? ` · ${s.ril100}` : ""} · {s.distanceKm.toFixed(1)} km</span></div><button disabled={selected || saving} className={styles.addStation} onClick={() => addNearbyStation(s)}>{selected ? "✓ übernommen" : "+ übernehmen"}</button></div>; })}</div> : <div className={styles.emptySmall}>Keine DB-Station im Suchradius gefunden.</div>}</section>}
      ...
      <footer className={styles.footer}><button className={styles.cancel} disabled={saving} onClick={() => setOpen(false)}>Abbrechen</button><button className={styles.primary} disabled={saving || !location || (railwayInfrastructure.candidates.length > 0 && !selectedRoute)} onClick={() => void save()}>{saving ? "Speichere…" : "Übergang speichern"}</button></footer>
    </aside></div>}
  </main>;
}
...
function formatRelative(value: string) { const diff = new Date(value).getTime() - Date.now(); if (diff <= 0) return "jetzt"; const min = Math.ceil(diff / 60000); return `in ${min} Min.`; }
...
function RouteMap({ lat, lon, candidates, selectedKey, onSelect }: { lat: number; lon: number; candidates: RailwayCandidate[]; selectedKey: string; onSelect: (candidate: RailwayCandidate) => void }) {
