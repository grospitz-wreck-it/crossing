...
export default function CrossingsAdmin() {
  ...
  function mapEmbedUrl() { if (!location) return ""; const lat = Number(location.lat), lon = Number(location.lon), dLat = 0.003, dLon = 0.005; return `https://www.openstreetmap.org/export/embed.html?bbox=${lon - dLon}%2C${lat - dLat}%2C${lon + dLon}%2C${lat + dLat}&layer=mapnik&marker=${lat}%2C${lon}`; }
    ...
    try {
      ...
      if (data.location) {
        try {
          ...
          if (requestId === lookupRequest.current) setRailwayInfrastructure(infraData || { status: "ERROR", candidates: [] });
