...
type CandidateStation = { eva: string; stationName: string; lat: number; lon: number; distanceKm: number; lines: string[] };
...
export default function CrossingsAdmin() {
  const [crossings, setCrossings] = useState<Crossing[]>([]), [open, setOpen] = useState(false), [saving, setSaving] = useState(false), [coords, setCoords] = useState(""), [nearest, setNearest] = useState<any[]>([]), [stations, setStations] = useState<CandidateStation[]>([]), [location, setLocation] = useState<any>(null), [lookupError, setLookupError] = useState("");
  ...
  function addStation(candidate?: CandidateStation) { const station: Station = { ...emptyStation(), eva: candidate?.eva || "", stationName: candidate?.stationName || "" }; setForm((f) => ({ ...f, stations: [...f.stations.filter((s) => s.eva.trim()), station] })); }
  async function findNearest() {
    if (!coords.trim()) return;
    setLookupError(""); setStations([]);
    ...
    setLocation(data.location); setNearest(data.nearest || []); setStations(data.stations || []);
  ...
  async function save() {
    ...
    try {
      ...
      await load(); setOpen(false); setNearest([]); setStations([]); setLocation(null); setCoords("");
  ...
  return <main className={styles.page}>
    ...
    {open && <div className={styles.backdrop} onMouseDown={() => setOpen(false)}><aside className={styles.drawer} onMouseDown={(e) => e.stopPropagation()}>
      ...
      <div className={styles.content}>
        <section><label>Google Maps / Plus Code</label><div className={styles.inline}><input value={coords} onChange={(e) => setCoords(e.target.value)} placeholder="z. B. 6F25+VRJ Rödinghausen"/><button className={styles.secondary} onClick={findNearest}>Standort prüfen</button></div><small>Akzeptiert Google-Maps-Plus-Codes wie „6F25+VRJ Rödinghausen“ sowie GPS-Koordinaten. Danach werden der nächste bekannte Übergang und nahe DB-Bahnhöfe ermittelt.</small>{lookupError && <div className={styles.error}>{lookupError}</div>}{location && <div className={styles.location}><strong>Standort erkannt</strong><span>{Number(location.lat).toFixed(6)}, {Number(location.lon).toFixed(6)} · {location.source === "plus-code-recovered" ? "Plus Code aufgelöst" : location.source === "plus-code" ? "Plus Code" : "GPS"}</span></div>}{nearest.length > 0 && <div className={styles.matches}>{nearest.map((n) => <div className={styles.match} key={n.id}><strong>{n.name}</strong><span>{n.distanceKm.toFixed(2)} km · EVA {n.eva}</span></div>)}</div>}</section>
        {stations.length > 0 && <section><label>DB-Bahnhöfe in der Umgebung</label><small>Die Liste kommt aus db.transport.rest. Bahnhöfe mit gemeinsamen Linien sind besonders interessant für die spätere Fahrplananalyse.</small><div className={styles.stationCandidates}>{stations.map((s) => <div className={styles.candidate} key={s.eva}><div><strong>{s.stationName}</strong><span>EVA {s.eva} · {s.distanceKm.toFixed(1)} km{s.lines.length ? ` · ${s.lines.slice(0,4).join(", ")}` : ""}</span></div><button onClick={() => addStation(s)}>+ übernehmen</button></div>)}</div></section>}
        ...
        <div className={styles.future}><strong>DB-Fahrplananalyse</strong><span>Die vorgeschlagenen EVAs werden später genutzt, um Zugläufe, Haltezeiten und Echtzeitinformationen aus der DB-Datengrundlage zusammenzuführen.</span></div>
