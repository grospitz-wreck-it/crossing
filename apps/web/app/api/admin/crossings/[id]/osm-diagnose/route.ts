...
function distanceMeters(a: Point, b: Point) {
  const scale = 111320;
  const x = (a.lon - b.lon) * scale * Math.cos((a.lat * Math.PI) / 180);
  const y = (a.lat - b.lat) * scale;
...
function routeDistanceToWay(route: Point[], geometry: Point[]) {
  ...
  for (const point of route) {
    ...
    for (let i = 1; i < geometry.length; i += 1) {
      best = Math.min(best, pointSegmentDistance(point, geometry[i - 1], geometry[i]));
    }
...
function routePoints(train: Train): Point[] {
  ...
  for (const stop of train.route || []) {
    const lat = Number(stop?.lat ?? stop?.latitude ?? stop?.location?.latitude);
    const lon = Number(stop?.lon ?? stop?.longitude ?? stop?.location?.longitude);
    if (Number.isFinite(lat) && Number.isFinite(lon)) result.push({ lat, lon });
  }
  return result;
}
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const crossingResult = await db.execute({
    sql: `SELECT id,name,eva,lat,lon,observation_evas,required_route_stops FROM crossings WHERE id = ? LIMIT 1`,
    args: [id],
  });
  ...
  const mappingResult = await db.execute({
    sql: `SELECT crossing_osm_id,railway_way_id,crossing_node_index,way_direction,tags_json FROM osm_crossing_rail_ways WHERE crossing_osm_id IN (SELECT osm_crossing_id FROM crossing_osm_links WHERE crossing_id = ?)`,
    args: [id],
  }).catch(() => ({ rows: [] as any[] }));
  const linkResult = await db.execute({
    sql: `SELECT osm_crossing_id,match_method,confidence FROM crossing_osm_links WHERE crossing_id = ? LIMIT 1`,
    args: [id],
  }).catch(() => ({ rows: [] as any[] }));

  const mappedWays = (mappingResult.rows as any[]).map((row) => ({
    crossingOsmId: Number(row.crossing_osm_id),
    railwayWayId: Number(row.railway_way_id),
    crossingNodeIndex: row.crossing_node_index == null ? null : Number(row.crossing_node_index),
    wayDirection: String(row.way_direction || "unknown"),
  ...
  let osmEndpoint = "";
  try {
    const osm = await fetchOverpass(Number(crossing.lat), Number(crossing.lon));
    osmNodes = osm.crossings;
    osmEndpoint = osm.endpoint;
  } catch (error) {
    osmEndpoint = error instanceof Error ? error.message : String(error);
  }
  ...
  for (const eva of evas.slice(0, 6)) {
    try {
      ...
      timetableErrors.push({ eva, error: error instanceof Error ? error.message : String(error) });
    }
  }
  ...
  const trainMatches = [...trainsByKey.values()].map((train) => {
    ...
    const matches = ways.map((way) => {
      ...
      return {
        railwayWayId: way.osmId,
        ref: way.ref,
        meanDistanceMeters: Number.isFinite(meanDistanceMeters) ? Math.round(meanDistanceMeters) : null,
        score: Number(proximity.toFixed(3)),
      };
    ...
    return {
      ...
      routeStations: (train.route || []).map((stop: any) => stop?.name || stop?.stationName || stop?.eva).filter(Boolean),
  ...
  return Response.json({
    crossing: { id: String(crossing.id), name: String(crossing.name), lat: Number(crossing.lat), lon: Number(crossing.lon) },
    osm: { linkedCrossingId: crossingOsmId, link: linkResult.rows[0] || null, discoveredNearbyCrossings: osmNodes.map((x: any) => ({ id: Number(x.id), lat: Number(x.lat), lon: Number(x.lon), tags: x.tags || {} })), ways: ways.map(({ geometry, ...rest }) => rest), endpoint: osmEndpoint },
    trains: trainMatches,
    timetableErrors,
    note: "Diagnostic only. No prediction is changed by this endpoint.",
  });
}
