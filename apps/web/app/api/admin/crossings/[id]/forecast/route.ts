...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Sicherheitsgrenze: Ein getStationTimetable()-Aufruf erzeugt bei hoursAhead=4
  // vier /plan-Requests plus einen /fchg-Request. Die gemeinsame 60-Sekunden-
  // Cache-Schicht in getStationTimetable verhindert Doppelabfragen; zusätzlich
  // begrenzen wir die Zahl der unterschiedlichen EVAs pro Forecast.
  ...
  if (throughRules.length) {
    try {
      ...
      for (const station of new Map(throughRules.map((rule: any) => [String(rule.observationEva), rule])).values()) {
        stationResults.push({ eva: String((station as any).observationEva), stationName: String((station as any).observationStation || (station as any).observationEva), role: "rule", rule: true, ok: true });
      }
      for (const train of throughTrains) {
        ...
        const candidate = {
          ...
          crossingTime: crossingTime.toISOString(),
          closeAt: new Date(crossingTime.getTime() - Number(crossing.close_offset_seconds || 80) * 1000).toISOString(),
          openAt: new Date(crossing.open_offset_seconds || 20).toISOString(),
          etaSeconds,
  ...
  return Response.json({
    ...
    rules: { requiredRouteStops, throughRules, diversionRules: jsonArray(crossing.diversion_rules), rerouteWatchRules: jsonArray(crossing.reroute_watch_rules), apiProtection: { maxDirectObservationStations: MAX_DIRECT_OBSERVATION_STATIONS, maxRuleStations: MAX_RULE_STATIONS } },
  });
}
