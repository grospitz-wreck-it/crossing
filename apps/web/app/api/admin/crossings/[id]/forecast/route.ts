...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  for (const eva of observationEvas) {
    try {
      const events = await getStationTimetable(eva, FORECAST_TIMETABLE_HOURS);
  ...
  if (throughRules.length) {
    try {
      ...
      for (const rule of throughRules) { const eva = String(rule.observationEva || "").trim(); if (eva && !uniqueRuleStations.has(eva)) uniqueRuleStations.set(eva, rule); }
      for (const [eva, rule] of uniqueRuleStations) {
        let count = stationEventCounts.get(eva);
        if (count === undefined) {
          try { const events = await getStationTimetable(eva, FORECAST_TIMETABLE_HOURS); count = events.length; stationEventCounts.set(eva, count); }
          catch { count = 0; }
        }
        stationResults.push({ eva, stationName: stationNameByEva.get(eva) || String(rule.observationStation || eva), role: "rule", rule: true, ok: true, count });
      }
