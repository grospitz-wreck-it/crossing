...
async function tryOverpass(lat: number, lon: number) {
  // Resolve route relations FROM THE LOCAL RAILWAY WAYS only. This prevents unrelated nearby lines.
  const query = `[out:json][timeout:5];way(around:220,${lat},${lon})[railway~"^(rail|tram|light_rail)$"]->.localWays;.localWays out geom;rel(bw.localWays)[type=route][route~"^(train|tram|light_rail|railway|tracks)$"]->.localRoutes;.localRoutes out body;`;
  // Keep this request bounded: the admin UI must never wait indefinitely for OSM.
