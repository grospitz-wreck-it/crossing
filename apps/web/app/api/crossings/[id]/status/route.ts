...
async function loadCrossing(id: string): Promise<any | null> {
  try {
    ...
    let stationRows: any[] = [];
    try { const stations = await db.execute({ sql: `SELECT eva,station_name,role FROM crossing_station_links WHERE crossing_id = ? ORDER BY sort_order ASC`, args: [id] }); stationRows = stations.rows as any[]; } catch {}
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const throughResults = await Promise.all(throughTrains.map(async (train: any) => ({ train, allowed: await allowTrainForCrossing(crossing.id, train, "through") })));
