...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Dynamische BÜs haben häufig keinen eigenen EVA. In diesem Fall müssen
  ...
  try {
    ...
    for (const train of reroutedTrains) {
      ...
      const etaSeconds = Math.floor((crossingTime.getTime() - now) / 1000);
      if (etaSeconds <= 0) continue;
      if (trains.some((item) => `${item.category}-${item.journeyNumber}` === key)) continue;
